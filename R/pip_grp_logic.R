...
pip_grp_logic <- function(
  ...
  # STEP 2: filter countries and years                             ####
  ...
  if (all(lcv$off_alt_agg == "off")) {
    # Users only request the official regions
    ### Early return -----------
    res <- pip_grp(
      country = country,
      year = year,
    ...
    fg_pip_master[is.na(mean), mean := predicted_mean_ppp]
  ...
  ## STEP 3.3 Compute needed regional aggregates -------
  ...
  md_grp <- merge(md_grp, cl, by = "country_code", all.x = TRUE)
...
pip_grp_helper <- function(
  ...
  # Filter countries
  keep_countries <- fg_pip[["country_code"]] %chin%
    lcv_country |
  ...
  if (group_by != "none") {
    ...
    out <- pip_aggregate(df = out, return_cols = lkup$return_cols$pip_grp)
    out <- estimate_type_var(out, lkup)
  }
