...
rg_pip_old <- function(country,
  ...
  # load data
  lt <- load_data_list_old(metadata)

  # parallelization
  # res <- get_pov_estimates(lt, povline = povline)

  # Regular lapply
  res <- lapply(lt, process_dt_old, povline = povline)
...
# OLD: Efficient FGT calculation for a data.table and vector of poverty lines
#' Title
#'
...
process_dt_old <- function(dt, povline) {
  dt[, compute_fgt_dt_old(.SD, "welfare", "weight", povline),
     by = .(file, reporting_level)]
}

#' OLD: load survey year files and store them in a list
#'
