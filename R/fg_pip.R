...
fg_pip <- function(
  ...
  if (nrow(metadata) == 0) {
    return(list(
      main_data = pipapi::empty_response_fg,
  ...
  if (!is.null(popshare)) {
    ...
    LDTg <- format_lfst(lfst = lfst, dict = dict)
    ...
    fgt <- fgt_cumsum(LDTg = LDTg, tpop = tpop, povline = povline) |>
  ...
  tmp_metadata[y == 0, distribution_type := "mixed"][,
    y := NULL
  ]
...
fg_standardize_cache_id <- function(
  ...
  out <- ifelse(
    grepl("|", interpolation_id, fixed = TRUE),
    gsub(
      paste0("_", unique(reporting_level), collapse = '|'),
      '',
      interpolation_id
    ),
    cache_id
