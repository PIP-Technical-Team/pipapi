...
test_that("year selection is working", {
  ...
  check <- max(lkup$svy_lkup[country_code == "PRY"]$reporting_year)
...
test_that("Censoring for regional aggregations is working", {
  skip("this test does not apply to pip anymore. Only to pip_grp")
  lkup2 <- lkup
  censored <- list(
    regions = data.frame(
      region_code    = "SSA",
...
test_that("pop_share option is returning consistent results for single grouped distributions", {
  ...
  # Zero headcount get the min of the distribution as povline
  ps <- pip(
    country = country,
    year = year,
    popshare = 0,
