...
test_that("filter_md works when year is passed as a character", {
  ...
  # single year
  out <- filter_md(md = md, ctr_alt_agg = ctr_alt_agg, year = "1983")

  expect_equal(length(unique(out$year)), 1)
  expect_true(all(unique(out$country_code) %in% ctr_alt_agg))

  # 2 years
  out <- filter_md(md = md, ctr_alt_agg = ctr_alt_agg, year = c("1983", "2000"))

  expect_equal(length(unique(out$year)), 2)
  expect_true(all(unique(out$country_code) %in% ctr_alt_agg))

  # All years
  out <- filter_md(md = md, ctr_alt_agg = ctr_alt_agg, year = "ALL")
...
test_that("get_md_vars works as expected", {
  ...
  md <- aux_files$missing_data
  ctr_alt_agg <- cl$country_code[cl$region_code == "SSF"]
  year <- 1981
  off_alt_agg <- "both"
  user_off_reg <- "SSF"
  tmp <- get_md_vars(
...
test_that("get_grp_to_compute works as expected", {
  ...
  tmp <- get_grp_to_compute(user_off_reg, md_off_reg, year, md_year)
...
test_that("get_impl_ctrs works as expected", {
  ...
  expect_equal(tmp, ctrs$country_code[ctrs$region_code == "SSF"])
...
test_that("get_ctr_alt_agg works as expected", {
  ...
  user_alt_agg <- alt_agg
  tmp <- get_ctr_alt_agg(
    user_alt_gt = user_alt_gt,
    user_alt_gt_code = user_alt_gt_code,
    user_alt_agg = user_alt_agg,
    cl = cl
  )
  expect_equal(tmp, cl$country_code[cl$region_code == "SSF"])
...
test_that("select_off_alt_agg works as expected", {
  ...
  tmp <- select_off_alt_agg(
    user_gt = user_gts[!user_gts %in% off_gt],
    off_gt = off_gt
  )
  expect_equal(tmp, "alt")
})
...
test_that("create_vector_countries output the expected object", {
  country <- "ALL"
  year = "2010"
  out <- create_countries_vctr(country = country, year = year, lkup = lkup)
...
test_that("create_vector_countries Returns correct results when country = ALL", {
  ...
  expect_equal(
    ...
    aux_files$regions$region_code[
      aux_files$regions$grouping_type %in% c("regionpcn", "africa_split")
    ]
  )
...
test_that("create_vector_countries Returns correct results when country = alternative aggregate", {
  ...
  expect_equal(out$md_off_reg, "SSF")
  # Correct instructions to compute aggregates are correct
...
test_that("create_vector_countries returns correct results when country = offical aggregate", {
  # Correctly selects official regions
  country <- c("LCN", "MEA")
  year = "ALL"
  out <- create_countries_vctr(country = country, year = year, lkup = lkup)

  # Returns official regions
  expect_equal(out$user_off_reg, country)
  # Returns no alternative region
  expect_true(is.null(out$user_alt_agg))
  # Correct instructions to compute aggregates are correct
  expect_true(is.null(out$grp_use))
  expect_equal(out$off_alt_agg, "off")
...
test_that("create_vector_countries returns correct results when country = aggregates are mixed", {
  # Correctly selects mixed aggregates (official + alternative)
  off_country <- c("LCN", "MEA")
  alt_country <- "AFE"
